globals [ px py xcorplus ycorplus] ;; globalne varijable za praćenje položaja i smjera kretanja

breed [cars car]                   ;; kreira se "cars" kao tip agenta
breed [obstacles x]                ;; kreira se "obstacles" kao tip agenta

to setup                           ;; postavljanje početnih uvjeta
  ...
  [
    ...
    move-to one-of patches with [pcolor = gray and (one-of other cars-here = nobody) ]
                                   ;;postavljaju se na cestu (siva ćelija)
    set heading 90                 ;; okreću se udesno
  ]
...
to walk                            ;; funkcija kretanja vozila (agentska funkcija)
  if ([pcolor] of patch 6 5 = green)
                                   ;;ako je na semaforu zeleno, vozila se kreću
  [
    set xcorplus xcor + 1          ;; inkrementira xcorplus
    set ycorplus ycor + 1          ;; inkrementira ycorplus
    ifelse any? obstacles-on (patch xcorplus ycor )
    [
      ifelse ([pcolor] of patch xcor ycorplus = gray)  ;; ako je sljedeća ćelija slobodna
      [
        show list xcor ycor
        rt 270
        fd 1
...
to stoplight                 ;; funkcija semafora (funkcija okruženja)
  ifelse ticks mod 100 > 50
                             ;; svakih 50 tickova mijenja se svjetlo
  [
    ask patch 6 6
    [
      set pcolor black
...
to draw-road                  ;; funkcija crtanja ceste
  set px -6
  set py -3
  while [py < 4]
  [
