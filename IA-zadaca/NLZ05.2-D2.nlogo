globals [dirt-probability dirt-count ]
...
to go
  ...
  set dirt-probability random 100
                          ; genereira se slucajan broj manji od 100
  if dirt-probability >= 95
                          ; i ako je veci ili jednak 95 (5%)
  [
...
to clean                 ;; funkcija kretanja usisivaca
  ifelse(any? patches with [pcolor = grey])
  [
    let closest-dirt min-one-of patches with [pcolor = grey] [distance myself]
    face closest-dirt
    fd 1
  ...
  [
    if (xcor = 6) and (ycor = 6)
                         ;; kada dodje u gornju desu celiju (a prostorija nije cista)
    [
      setxy -6 -6        ;; usisavac se vraca u donju lijevu celiju i cisti iz pocetka
    ]
  ]
end
@#$#@#$#@
