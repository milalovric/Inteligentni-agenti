...
to clean                 ;; funkcija kretanja usisivacavozila
  ifelse (count patches with [pcolor = grey] > 0)  ;; probjerava ima li jos prljavih mjesta u prostoriji
  [
    let closest-dirt min-one-of patches with [pcolor = grey] [distance myself]  ;; pronalazi najblize prljavo mjesto
    face closest-dirt                                                           ;; okrece usisivac prema najblizem prljavom mjestu
    fd 1
    ...
    [
      type "Moving to the closest dirt" print ""
    ]
  ]
  [
  ifelse (xcor = 6) and (heading = 90)
                         ;; okretanje na desnom rubu prostorije
  [
    set heading 0
    fd 1
    set heading 270
  ...
  [
    ifelse (xcor = -6) and (heading = 270)
                         ;; okretanje na lijevom rubu prostorije
    [
      set heading 0
      fd 1
      set heading 90
